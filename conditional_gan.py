...
def create_mnist_dataloader(n_batch_: int = 64):
    # setting
    ...
    # create dataloader
    train_loader = DataLoader(train_set, batch_size=n_batch_, shuffle=True)
    return train_loader


def plot_comparison(real_image_: torch.Tensor, fake_image_: torch.Tensor,
                    epoch_: int, image_num_: int = 10) -> None:
    dir_path = 'result/gan_comparison_images/'

    if not os.path.exists(dir_path):
        os.makedirs(dir_path)

    real_image_ = real_image_.cpu().numpy()
    fake_image_ = fake_image_.cpu().detach().numpy()

    fig = plt.figure(figsize=(20, 4))
    gs = gridspec.GridSpec(2, image_num_)
    gs.update(wspace=0.05, hspace=0.05)

    for i in range(image_num_):
        ax = plt.subplot(gs[0, i])
        plt.imshow(real_image_[i].reshape(28, 28), cmap='gray')
        ax.axis('off')

        ax = plt.subplot(gs[1, i])
        plt.imshow(fake_image_[i].reshape(28, 28), cmap='gray')
        ax.axis('off')

    plt.savefig(
        f'./result/gan_comparison_images/epoch_{epoch_}.pdf', bbox_inches='tight'
    )
    plt.close(fig)


def create_gif(gif_name_: str = None, fps_: float = 5.0) -> None:
    # setting
    root_dir = './result/GIF'
...
def plot_loss(d_real_: list, d_fake_: list, d_mean_: list, g_: list) -> None:
    d_real_loss = np.array(d_real_)
    d_fake_loss = np.array(d_fake_)
    d_mean_loss = np.array(d_mean_)
    g_loss = np.array(g_)
...
class Generator(nn.Module):
    def __init__(
            self,
            z_dim_: int = 100,
            init_linear_size_: tuple = (64, 7, 7),
            conv_kernel_size_: None or list = None,
            conv_kernel_filter_: None or list = None,
            dropout_rate_: float = 0.0,
            batch_norm_: bool = True,
            activation_: str = 'relu',
            output_size_: tuple = (1, 28, 28)
    ) -> None:
        """Discriminator
        The number of elements in the list
        (conv_kernel_size, conv_kernel_filter) must be same.
        ...
        super().__init__()
        batch_norm_layers = []
        dropout_layer = nn.Dropout2d(p=dropout_rate_)
        activate_fn = nn.ReLU()
        init_linear_features = \
            init_linear_size_[0] * init_linear_size_[1] * init_linear_size_[2]
        if conv_kernel_size_ is None:
            conv_kernel_size_ = [3, 3, 3]
        if conv_kernel_filter_ is None:
            conv_kernel_filter_ = [128, 64, 1]
        if activation_ == 'leaky_relu':
            activate_fn = nn.LeakyReLU(inplace=True)
        if batch_norm_:
            batch_norm_layers.append(nn.BatchNorm1d(init_linear_features))
            for features in conv_kernel_filter_[0:-1]:
        ...
        self._conv_kernel_size = conv_kernel_size_
        self._conv_kernel_filter = conv_kernel_filter_
        self._init_linear_size = init_linear_size_
    ...
    def forward(self, z_: torch.Tensor) -> torch.Tensor:
        x = self._init_linear(z_)
...
def train_gan(
    ...
    for epoch in tqdm(range(epoch_num_)):
        ...
        for x, _ in data_loader_:
            ...
            plot_comparison(x, fake_x, epoch + 1, image_num_=10)

        d_real_losses.append(d_real_l / len(data_loader_))
        d_fake_losses.append(d_fake_l / len(data_loader_))
        d_mean_losses.append(d_mean_l / len(data_loader_))
        g_losses.append(g_l / len(data_loader_))
...
if __name__ == '__main__':
    ...
    batch_size = 256
    train_dataloader = create_mnist_dataloader(n_batch_=batch_size)
    ...
    generator = Generator(z_dim_=z_dim, init_linear_size_=(64, 7, 7),
                          conv_kernel_size_=[5, 5, 5, 5],
                          conv_kernel_filter_=[128, 64, 64, 1],
                          dropout_rate_=0.0, batch_norm_=True,
                          activation_='relu', output_size_=(1, 28, 28))
    ...
    # training gan
    epoch_num = 100
    train_gan(epoch_num_=epoch_num, data_loader_=train_dataloader,
              discriminator_=discriminator, generator_=generator,
              d_optimizer_=discriminator_optimizer,
              g_optimizer_=generator_optimizer, z_dim_=z_dim,
    ...
    create_gif(gif_name_=gif_name, fps_=5)
